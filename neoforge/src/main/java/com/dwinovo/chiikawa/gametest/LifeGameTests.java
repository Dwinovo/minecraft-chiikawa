...
public final class LifeGameTests {
    ...
    public static void wild_pets_spawn_where_the_data_pack_says(GameTestHelper helper) {
        ...
        helper.assertTrue(spawns(biomes.getValueOrThrow(Biomes.PLAINS), usagi), "no Usagi turns up on the plains");
        helper.assertFalse(spawns(biomes.getValueOrThrow(Biomes.OCEAN), usagi), "Usagi turns up at sea, where no list puts it");
        helper.succeed();
    }
