...
public final class WeaponGameTests {
    ...
    public static void rakkos_sword_makes_a_fencer(GameTestHelper helper) {
        ...
        helper.assertTrue(new ItemStack(sword).is(InitTag.ENTITY_FENCER_TOOLS), Component.literal("Rakko's sword is not a fencer's tool"));
        AbstractPet pet = holding(wildPet(helper, new BlockPos(3, STAND, 3)), sword);
        helper.assertTrue(pet.getPetJobId() == InitRegistry.FENCER_ID, Component.literal("Rakko's sword did not make a fencer"));
        helper.succeed();
    }
    ...
    public static void a_wild_rakko_is_born_with_its_sword(GameTestHelper helper) {
        ...
        helper.assertTrue(rakko != null, Component.literal("nothing spawned"));
        helper.assertTrue(rakko.getMainHandItem().is(InitItems.RAKKO_SWORD.get()),
            Component.literal("a wild Rakko turned up without its sword: " + rakko.getMainHandItem()));
        helper.succeed();
    }
    ...
    public static void nobody_makes_rakkos_sword(GameTestHelper helper) {
        ...
        helper.assertFalse(helper.getLevel().recipeAccess().getRecipes().stream()
                .anyMatch(recipe -> recipe.value().display().stream().anyMatch(display -> display.result()
                    .resolveForStacks(context).stream().anyMatch(stack -> stack.is(InitItems.RAKKO_SWORD.get())))),
            Component.literal("Rakko's sword can be made"));
        helper.succeed();
    }
    ...
    public static void no_shop_deals_in_rakkos_sword(GameTestHelper helper) {
        Item sword = InitItems.RAKKO_SWORD.get();
        helper.assertFalse(ShopCatalogs.all().isEmpty(), Component.literal("no price list loaded, so this proves nothing"));
        for (ShopCatalog catalog : ShopCatalogs.all().values()) {
            helper.assertTrue(catalog.sale(sword).isEmpty() && catalog.purchase(sword).isEmpty(),
                Component.literal("a shop deals in Rakko's sword"));
        }
        helper.succeed();
    }
}
