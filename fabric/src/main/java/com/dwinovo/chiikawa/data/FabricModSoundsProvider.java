...
public class FabricModSoundsProvider implements DataProvider {
    ...
    public CompletableFuture<?> run(CachedOutput cache) {
        ...
        // Straight under the namespace: a path provider always puts a folder of its kind in between.
        return DataProvider.saveStable(cache, root,
            output.getOutputFolder(PackOutput.Target.RESOURCE_PACK).resolve(Constants.MOD_ID).resolve("sounds.json"));
    }
