...
public abstract class ChiikawaEntityRenderer<T extends Entity> extends EntityRenderer<T, ChiikawaRenderState> {
    ...
    private void drawLabel(ChiikawaRenderState state, PoseStack poseStack, MultiBufferSource bufferSource,
                           float extraHeight) {
        if (!overHead(state.labelAttachment, poseStack, extraHeight)) {
            return;
        }
        state.label.draw(new WorldSurface(poseStack, bufferSource, getFont()), 0, 0);
        poseStack.popPose();
    }
    ...
    private void extractSpeech(T entity, ChiikawaRenderState state, float partialTick) {
        ...
        pet.getSpeech().ifPresent(speech -> {
            ...
            state.speech = Component.translatable(speech.line()).getString();
            state.speechAlpha = Mth.clamp(left / SPEECH_FADE_TICKS, 0.0F, 1.0F);
            state.speechAttachment = entity.getAttachments().getNullable(EntityAttachment.NAME_TAG, 0, entity.getViewYRot(partialTick));
        });
    }
    ...
    private void drawSpeech(ChiikawaRenderState state, PoseStack poseStack, MultiBufferSource bufferSource) {
        ...
        DrawSurface surface = new WorldSurface(poseStack, bufferSource, getFont(), state.speechAlpha);
        float below = (state.nameTag != null ? LABEL_LINE : 0.0F)
            + (state.label != null ? (state.label.height(surface) + UiStyle.GAP) * LABEL_SCALE : 0.0F);
        if (!overHead(state.speechAttachment, poseStack, below)) {
            return;
        }
        Bubble bubble = new Bubble(state.speech);
