...
public final class WorldSurface implements DrawSurface {
    ...
    public void fillRect(int x, int y, int width, int height, int argb) {
        ...
        collector.submitCustomGeometry(pose, RenderTypes.textBackground(), (at, consumer) -> {
            ...
            consumer.addVertex(at, x, y + height, z).setColor(argb).setLight(FULL_BRIGHT);
            consumer.addVertex(at, x + width, y + height, z).setColor(argb).setLight(FULL_BRIGHT);
            consumer.addVertex(at, x + width, y, z).setColor(argb).setLight(FULL_BRIGHT);
            consumer.addVertex(at, x, y, z).setColor(argb).setLight(FULL_BRIGHT);
        });
    }
