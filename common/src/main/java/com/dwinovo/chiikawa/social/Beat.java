...
public record Beat(Optional<String> animation, Optional<PetReaction> reaction, Optional<VoiceMoment> voice) {
    public static final MapCodec<Beat> MAP_CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(
        ExtraCodecs.strictOptionalField(Codec.STRING, "animation").forGetter(Beat::animation),
        ExtraCodecs.strictOptionalField(PetReaction.CODEC, "reaction").forGetter(Beat::reaction),
        ExtraCodecs.strictOptionalField(VoiceMoment.CODEC, "voice").forGetter(Beat::voice)
