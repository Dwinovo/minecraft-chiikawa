 ...
 *
 * <p>Past the leash, a pet that is not already walking somewhere walks back to the
 * anchor center; intents that steer the pet themselves end on their own once their
 * target leaves the leash. Anchors of pets that cannot move, or are leashed or riding,
 * never pull.
 *
 * <p>Falling so far behind that walking back will not do is not handled here:
 * {@link com.dwinovo.chiikawa.entity.PetFollowKeeper} fetches such a pet from the
 * server side, because a pet that has stopped thinking cannot fetch itself.
 */
public class AnchorLeashBehavior extends Behavior<AbstractPet> {
    ...
    protected boolean checkExtraStartConditions(ServerLevel level, AbstractPet pet) {
        ...
        return anchor.canMove() && !anchor.withinLeash(petPos)
            && !pet.getBrain().hasMemoryValue(MemoryModuleType.WALK_TARGET);
    }
